...
<div class="flex-grow-1 p-4">
    ...
    <form action="{{ route('admin.country-sections.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3 media-image" id="imageInput" style="display:none;">
            <label>Upload Main Image</label>
            <input type="file" name="image" class="form-control">
        </div>

        <div class="mb-3 media-image" style="display:none;">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const imageInputs = document.querySelectorAll('.media-image');
        const videoInput = document.getElementById('videoInput');

        mediaTypeSelect.addEventListener('change', function () {
            const value = this.value;
            imageInputs.forEach(input => input.style.display = value === 'image' ? 'block' : 'none');
